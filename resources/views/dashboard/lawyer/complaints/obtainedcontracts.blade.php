...
@section('title', 'العقود المحصلة')

@section('Content')


    <!-- Page Title -->
    <h1 class="text-1xl font-bold text-gray-800 mb-6">العقود المحصلة</h1>
    ...
    <div
        ...
        {{-- 🔍 Search Form --}}
        <form method="GET" action="{{ route('lawyer.complaints.obtainedcontracts') }}" class="w-full md:w-2/3">
            <div class="flex">
        ...
        <div class="flex gap-2">
            <a href="{{ route('lawyer.complaints.obtainedcontracts') }}"
                class="px-4 py-2 rounded-md border text-sm font-medium
                                                                                                                                            {{ request('filter') == null ? 'bg-[#1B7A75] text-white' : 'bg-white text-[#1B7A75] border-[#1B7A75]' }}">
                جميع المحصلة
            </a>

            <a href="{{ route('lawyer.complaints.obtainedcontracts', ['filter' => 'partial']) }}"
                class="px-4 py-2 rounded-md border text-sm font-medium
                                                                                                                                            {{ request('filter') == 'partial' ? 'bg-[#1B7A75] text-white' : 'bg-white text-[#1B7A75] border-[#1B7A75]' }}">
                محصلة جزئياً
            </a>

            <a href="{{ route('lawyer.complaints.obtainedcontracts', ['filter' => 'full']) }}"
                class="px-4 py-2 rounded-md border text-sm font-medium
                                                                                                                                            {{ request('filter') == 'full' ? 'bg-[#1B7A75] text-white' : 'bg-white text-[#1B7A75] border-[#1B7A75]' }}">
                محصلة كلياً
            </a>
        </div>
    ...
    <div
        ...
        <form method="GET" action="{{ route('lawyer.complaints.obtainedcontracts') }}"
            class="w-full md:w-auto flex gap-2 items-center">
            <input type="month" name="month" value="{{ request('month') }}"
                class="rounded-lg border border-[#1B7A75] text-gray-800 px-3 py-2 text-sm focus:ring-2 focus:ring-[#1B7A75]/40 focus:border-[#1B7A75] outline-none" />
            ...
            @if(request('month'))
                <a href="{{ route('lawyer.complaints.obtainedcontracts') }}"
                    class="text-sm text-[#1B7A75] underline hover:text-[#16615C]">
                    إعادة التعيين
                </a>
            @endif
        </form>
            ...
            <table class="data-table text-center">
                <thead>
                    <tr>
                        ...
                        <th style="font-size: 11px">تاريخ الطلب</th>
            ...
            <div class="bg-white rounded-lg shadow-strong w-full max-w-md mx-4 p-6 relative">
                ...
                <form id="statusForm" method="POST" action="">
                    ...
                    <div class="flex justify-end gap-3">
                        ...
                        <button type="submit"
                            class="btn prevent-double bg-[#1B7A75] hover:bg-[#16615C] text-white  f-12 hover-up">حفظ</button>
                    </div>
                </form>
            </div>
        </div>
